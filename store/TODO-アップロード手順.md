...
> **1.1 は 2026-09-08 に公開。** 投げ銭も同時に有効。承認は 09-05 ごろ、公開まで3日空いたので
> 次から**リリース方法は「自動」**にする。
>
> **1.1 は 2026-09-03 に審査提出。** バージョンと投げ銭の2項目を同時に出した。
> 投げ銭（消費型課金）はこれが初回なので、**アプリのバージョンと同時にしか出せない**。
>
...
| データ収集 | 収集しない |
| 価格 | 無料 |
| リリース方法 | ~~手動~~ → **次から「自動」**（2026-09-08 決定。承認されたらそのまま公開） |
