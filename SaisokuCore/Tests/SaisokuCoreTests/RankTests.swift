...
struct RankTests {
    ...
    @Test("称号ごとにLv1からLv5まである")
    func everyTitleHasFiveLevels() {
        for entry in RankLadder.table {
            #expect(entry.steps.count == 5, "\(entry.title) のレベル数が5でない")
        }
        #expect(RankLadder.all.count == RankLadder.table.count * 5)
        for rank in RankLadder.all { #expect((1...5).contains(rank.level)) }
    }
    ...
    func promotesExactlyOnThreshold() {
        #expect(RankLadder.rank(forFastestCount: 14)?.display == "見習い雀士 Lv4")
        #expect(RankLadder.rank(forFastestCount: 15)?.display == "見習い雀士 Lv5")
        #expect(RankLadder.rank(forFastestCount: 19)?.display == "見習い雀士 Lv5")
        #expect(RankLadder.rank(forFastestCount: 20)?.display == "手なり雀士 Lv1")
        #expect(RankLadder.rank(forFastestCount: 225)?.display == "速攻雀士 Lv3")
    }
    ...
    func topRankHasNoNext() {
        #expect(RankLadder.next(forFastestCount: 540) == nil)
        #expect(RankLadder.rank(forFastestCount: 540) == RankLadder.top)
        #expect(RankLadder.top.display == "神速雀士 Lv5")
        let next = RankLadder.next(forFastestCount: 19)
    ...
    func promotionIsReportedOnce() {
        ...
        let second = r.finishRound(wasFastest: true)    // 2回目は昇格なし
        #expect(second.promotedTo == nil)
        let third = r.finishRound(wasFastest: true)     // 3回でLv2
        #expect(third.promotedTo?.display == "見習い雀士 Lv2")
    }
    ...
    func decodesOldRecordsWithMissingFields() throws {
        ...
        #expect(r.rank?.display == "手なり雀士 Lv5", "42回は手なり雀士の最終段(40回)を超えている")
    }
